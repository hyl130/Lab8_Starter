describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let innerTxt = await shadowButton.getProperty('innerText');
    // Once you have the innerText property, use innerText['_remoteObject'].value to get the text value of it
    expect( innerTxt['_remoteObject'].value ).toBe("Remove from Cart");
  }, 2500);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let innerTxt = await cartCount.getProperty('innerText'); 
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
   ...
   let prodItems = await page.$$('product-item'); 
    ...
    let cartCount = await page.$('#cart-count');
    let innerTxt = await cartCount.getProperty('innerText'); 
    expect( innerTxt['_remoteObject'].value ).toBe("0"); 
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++){    
      ...
      let shadowButton = await shadowRoot.$('button'); 
      let innerTxt = await shadowButton.getProperty('innerText');
      expect( innerTxt['_remoteObject'].value ).toBe("Add to Cart"); 
    } 
